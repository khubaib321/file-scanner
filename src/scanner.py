...
def _should_ignore_dir(path: str, name: str, scan_hidden: bool) -> bool:
    return name in _IGNORE_DIRS or (not scan_hidden and name.startswith('.'))
            ...
            with _os.scandir(path) as it:
                for entry in it:
                    if (
                        ...
                        and not _should_ignore_dir(entry.path, entry.name, scan_hidden_dirs)
                    ):
